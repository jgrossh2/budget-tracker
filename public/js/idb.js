...
request.onupgradeneeded = function(event) {
    ...
    // create an object store (table) called `change_budget`, set it to have an auto incrementing primary key of sorts 
    db.createObjectStore('change_budget', { autoIncrement: true });
  };
// upon a successful 
  ...
  if (getAll.result.length > 0) {
    fetch('/api/transaction', {
      ...
      .then(serverResponse => {
        ...
        const transaction = db.transaction(['change_budget'], 'readwrite');
        // access the change_budget object store
        const budgetObjectStore = transaction.objectStore('change_budget');
